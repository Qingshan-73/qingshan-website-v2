  ...
  function initContent() {
    ...
    if (counters.length) {
      const runCounter = el => {
        ...
        const fmt = n => n.toLocaleString('en-US');   /* 千分位：10000 → 10,000 */
        if (prefersReduced || isNaN(target)) { el.textContent = fmt(target || 0) + suffix; return; }
        const duration = 1400;
        const start = performance.now();
        const tick = now => {
          const p = Math.min((now - start) / duration, 1);
          el.textContent = fmt(Math.round(easeOutCubic(p) * target)) + (p === 1 ? suffix : '');
